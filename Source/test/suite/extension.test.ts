...
const MockExtensionContext = {
	...
	globalStorageUri: Uri.file(MockGlobalStoragePath),
} as ExtensionContext;

const TestDownloadUri = Uri.parse(
	`https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf`
);
const TestDownloadFilename = `test.pdf`;
const TestDownloadUriWithSettings = Uri.parse(
	`https://api.github.com/repos/stuartleeks/pick-a-browser/releases/assets/95441770`
);
...
suite(`Integration Tests`, () => {
	...
	suiteSetup(async () => {
		const downloaderExtension = extensions.getExtension(
	...
	test(`Get item with no downloads`, async () => {
		await assert.rejects(
			fileDownloader.getItem(TestDownloadFilename, MockExtensionContext)
		);
	});

	test(`Get item with one download`, async () => {
		const downloadedUri: Uri = await fileDownloader.downloadFile(
			TestDownloadUri,
			TestDownloadFilename,
			MockExtensionContext
		);
		const getItemResult: Uri = await fileDownloader.getItem(
			TestDownloadFilename,
			MockExtensionContext
		);
	...
	test(`Get item with multiple downloads`, async () => {
		const downloadedItem1 = await fileDownloader.downloadFile(
			TestDownloadUri,
			`1.pdf`,
			MockExtensionContext
		);
		const downloadedItem2 = await fileDownloader.downloadFile(
			TestDownloadUri,
			`2.pdf`,
			MockExtensionContext
		);
		const downloadedItem3 = await fileDownloader.downloadFile(
			TestDownloadUri,
			`3.pdf`,
			MockExtensionContext
		);
		const getItem1 = await fileDownloader.getItem(
			`1.pdf`,
			MockExtensionContext
		);
		const getItem2 = await fileDownloader.getItem(
			`2.pdf`,
			MockExtensionContext
		);
		const getItem3 = await fileDownloader.getItem(
			`3.pdf`,
			MockExtensionContext
		);
	...
	test(`Overwrite existing folder`, async () => {
		const fullDownloadFilePath = path.join(
			MockGlobalStoragePath,
			`file-downloader-downloads`,
			`file`
		);
	...
	test(`tryGetItem with no downloads`, async () => {
		const getItemResult = await fileDownloader.tryGetItem(
			TestDownloadFilename,
			MockExtensionContext
		);
	...
	test(`tryGetItem with one download`, async () => {
		...
		const getItemResult: Uri | undefined = await fileDownloader.tryGetItem(
			TestDownloadFilename,
			MockExtensionContext
		);
	...
	test(`List items with no downloads`, async () => {
		const result: Uri[] =
			await fileDownloader.listDownloadedItems(MockExtensionContext);
	...
	test(`List items with multiple downloads`, async () => {
		...
		assert.deepStrictEqual(
			downloadedItem1.toString(),
			result[0].toString()
		);
		assert.deepStrictEqual(
			downloadedItem2.toString(),
			result[1].toString()
		);
		assert.deepStrictEqual(
			downloadedItem3.toString(),
			result[2].toString()
		);
	});
	...
	test(`Delete all items with no downloads`, async () => {
		await assert.doesNotReject(
			fileDownloader.deleteAllItems(MockExtensionContext)
		);
	});
		...
		const downloadProgressCallback = (
			...
			assert(
				reportedTotalBytes === 0 ||
					reportedDownloadedBytes <= reportedTotalBytes
			);
		};
		await fileDownloader.downloadFile(
			TestDownloadUri,
	...
	test(`Nonexistent website with correct Uri format`, async () => {
		try {
			await fileDownloader.downloadFile(
				Uri.parse(
					`https://c0de104c1e68625629646025d15a6129a2b4b6496cd9ceacd7f7b5078e1849ba.com`
				),
				TestDownloadFilename,
				MockExtensionContext
			);
	...
	test(`404 status code`, async () => {
		try {
			await fileDownloader.downloadFile(
				Uri.parse(
					`https://github.com/microsoft/cascadia-code/releases/download/v2005.15/doesntexist`
				),
				TestDownloadFilename,
				MockExtensionContext
			);
	...
	test(`Simple download from GitHub`, async () => {
		...
		const downloadedFile: Uri = await fileDownloader.downloadFile(
	...
	test(`Make executable`, async () => {
		...
		await fs.promises
			.access(downloadedFile.fsPath, fs.constants.X_OK)
			.catch((error: Error) => {
